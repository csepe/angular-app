...
@Component({
  selector: 'app-waves-generator',
  templateUrl: './waves-generator.component.pug',
  styleUrls: ['./waves-generator.component.scss']
})
export class WavesGeneratorComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const config1 = {
      nx: 40,
      ny: 1000,
      cscale: ["#2175D8", "#DC5DCE", "#CC223D", "#F07414", "#FDEE61", "#74C425"],
      darken: -1,
      angle: Math.PI / 3,
      timeCoef: 0.1
    };
    ...
    let config3 = {
      nx: 26,
      ny: 100,
      cscale: ["#1f476d", "#c25cd9", "#5a6afb", "#477bc0", "#88e26e"],
      darken: 1.5384104365825917,
      angle: 4.187197039767258,
      timeCoef: 0.13028021093769576
    };

    this.generateWave(config1, "canvas");
    //this.generateWave(config2, "canvas2");
  }


  generateWave(conf: any, canvasId: string) {
    ...
    class Polyline {
      ...
      updateGeometry() {
        this.points.forEach((p: any, i: any) => {
    ...
    let width, height;
    const { randFloat: rnd } = (THREE as any).Math;
    ...
    function init() {
      ...
      updateSize(canvas);
      window.addEventListener("resize", (updateSize(canvas) as any), false);
      canvas.addEventListener("click", initRandomScene);
    ...
    function initScene() {
      ...
      for (let i = 0; i < conf.nx; i++) {
        ...
        const polyline: any = new Polyline({ points });
        polylines.push(polyline);
        let scale = chroma.scale(conf.cscale).mode("lch");
        const material = new THREE.ShaderMaterial({
          uniforms: {
            ...
            uColor1: { value: new THREE.Color(scale(i / conf.nx).hex()) },
            uColor2: {
    ...
    function updateSize(canvas?: any) {
      width = canvas.getBoundingClientRect().width; //window.innerWidth / 2;
      height = canvas.getBoundingClientRect().height; //window.innerHeight;
